...
<form action="{{ route('register', ['step' => "detail"]) }}" class="SlideItem flex flex-col gap-6" method="POST">
    @csrf
    ...
    <div>
        ...
        <div class="text-[11px] text-slate-400 mt-2">
            Pastikan email sesuai di Plataran Sehat
        </div>
    </div>
    ...
    <div>
        <div class="group border focus-within:border-primary rounded-lg p-1 relative">
            <label class="text-slate-500 group-focus-within:text-primary text-xs absolute top-2 left-2">Asal Rumah Sakit / Instansi</label>
            <input type="text" name="instansi" id="instansi" class="w-full h-12 px-1 mt-3 outline-none bg-transparent text-sm text-slate-700" value="{{ $payload['email'] ?? '' }}" required />
        </div>
        <div class="text-[11px] text-slate-400 mt-2 hidden">
            Pastikan email sesuai di Plataran Sehat
        </div>
    </div>
