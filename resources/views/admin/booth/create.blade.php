<div class="fixed top-0 left-0 right-0 bottom-0 bg-black bg-opacity-75 flex items-center justify-center hidden z-30" id="CreateBooth">
    <form action="{{ route('admin.booth.store') }}" method="POST" class="bg-white shadow-lg rounded-lg p-10 w-8/12 mobile:w-10/12 flex flex-col gap-4 mt-4" enctype="multipart/form-data">
        ...
        <div class="flex items-center gap-4 mb-4">
            <h3 class="text-lg text-slate-700 font-medium flex grow">Tambah Booth</h3>
            <ion-icon name="close-outline" class="cursor-pointer text-3xl" onclick="toggleHidden('#CreateBooth')"></ion-icon>
        </div>

        <div class="grid grid-cols-2 mobile:grid-cols-1 gap-8 mt-2">
            <div class="flex flex-col gap-4">
                {{-- <div class="w-full aspect-[5/2] bg-slate-200 rounded-lg relative flex flex-col gap-1 items-center justify-center" id="CoverArea">
                    <ion-icon name="image-outline" class="text-2xl text-slate-700"></ion-icon>
                    <div class="text-xs text-slate-600 font-medium">{{ __('organizer.icon') }}</div>
                    <input type="file" name="icon" id="icon" class="absolute top-0 left-0 right-0 bottom-0 opacity-0 cursor-pointer" onchange="onChangeImage(this, '#CoverArea')" required>
                </div> --}}
                ...
                <div class="flex justify-center">
                    <div class="w-32 h-32 bg-slate-200 rounded-full relative flex flex-col gap-1 items-center justify-center" id="IconArea">
                        ...
                        <input type="file" name="icon" id="icon" class="absolute top-0 left-0 right-0 bottom-0 opacity-0 cursor-pointer" onchange="onChangeImage(this, '#IconArea')" required>
                    </div>
                </div>
        ...
        <div class="flex items-center justify-end gap-4 mt-4">
            <button class="p-3 px-6 rounded-lg text-sm bg-slate-200 text-slate-700" type="button" onclick="toggleHidden('#CreateBooth')">Batal</button>
