<div class="fixed top-0 left-0 right-0 bottom-0 bg-black bg-opacity-75 flex items-center justify-center hidden z-30" id="EditAdmin">
    <form action="{{ route('admin.admins.store') }}" method="POST" class="bg-white shadow-lg rounded-lg p-10 w-5/12 mobile:w-10/12 flex flex-col gap-4 mt-4">
        ...
        <div class="flex items-center gap-4 mb-4">
            <h3 class="text-lg text-slate-700 font-medium flex grow">Edit Admin</h3>
            <ion-icon name="close-outline" class="cursor-pointer text-3xl" onclick="toggleHidden('#EditAdmin')"></ion-icon>
        </div>
        ...
        <div>
            ...
            <input type="password" name="password" id="password" class="w-full h-14 px-4 border rounded-lg text-sm text-slate-600 outline-0" value="{{ old('password') }}" placeholder="Biarkan kosong jika tidak ingin mengubah">
        </div>

        <div class="flex flex-wrap gap-4">
            @foreach ($roles as $r => $role)
                <div class="cursor-pointer rounded-lg border shadow p-4 flex flex-col grow basis-96 gap-2 RoleItem {{ $r == 'admin' ? 'border-primary' : '' }}" id="{{ $r }}" onclick="chooseRole('#EditAdmin', '{{ $r }}')">
        ...
        <div class="flex items-center justify-end gap-4 mt-4">
            <button class="p-3 px-6 rounded-lg text-sm bg-slate-200 text-slate-700" type="button" onclick="toggleHidden('#EditAdmin')">Batal</button>
